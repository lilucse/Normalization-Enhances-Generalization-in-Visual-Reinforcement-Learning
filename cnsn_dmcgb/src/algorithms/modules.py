...
from algorithms.cnsn import CNSN,CrossNorm,SelfNorm
...
class SharedCNN_cnsn(nn.Module):
	def __init__(self, obs_shape, num_layers=11, num_filters=32,active_num=0,fix=False):
		super().__init__()
		assert len(obs_shape) == 3
		self.num_layers = num_layers
		self.num_filters = num_filters
		###cnsn config
		self.cn_num = num_layers
		self.active_num = active_num#int(num_layers/2)  # TODO 2..
		self.fix = fix
		crop = 'style'
		#crop = 'both'
		print('cn number: ',active_num)
		print('crop type: ',crop)
		beta = 1
		in_channels_list = [num_filters for n in range(num_layers)]
		self.cnsn_list = []
		...
		self.layers = [CenterCrop(size=84), NormalizeImg(), nn.Conv2d(obs_shape[0], num_filters, 3, stride=2),self.cnsn_list[0]]
		for _ in range(1, num_layers):
			self.layers.append(nn.ReLU())
			self.layers.append(nn.Conv2d(num_filters, num_filters, 3, stride=1))
			self.layers.append(self.cnsn_list[_])
		self.layers = nn.Sequential(*self.layers)
		self.eval()
		self.out_shape = _get_out_shape(obs_shape, self.layers)
		self.train()
	...
	def forward(self, x):
		if self.training and self.fix:
			self._enable_cross_norm_fix()
		if self.training and self.fix == False:
	...
	def _enable_cross_norm_fix(self):
		active_cn_idxs = [i for i in range(0,self.cn_num)]
		for idx in active_cn_idxs:
			self.cnsn_list[idx].crossnorm.active = True

# class HeadCNN_cnsn(nn.Module):
# 	def __init__(self, in_shape, num_layers=0, num_filters=32):
# 		super().__init__()
# 		self.layers = []
# 		for _ in range(0, num_layers):
