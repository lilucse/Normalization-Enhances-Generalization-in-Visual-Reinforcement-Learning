...
eval_mode_list = ['color_hard','video_hard','video_easy']
def evaluate(env, agent, video, num_episodes, L, step, test_env=False,index=-1):
	agent.eval()
	episode_rewards = []
	for i in range(num_episodes):
		...
		if L is not None:
			if test_env == False:
				...
				_test_env = '_'+eval_mode_list[index]
	...
	for step in range(start_step, args.train_steps+1):
		if done:
			...
			if step % args.eval_freq == 0:
				...
				for eval_index in range(len(eval_env_list)):
					evaluate(eval_env_list[eval_index], agent, video, args.eval_episodes, L, step, test_env=True,index=eval_index)
				L.dump(step)
