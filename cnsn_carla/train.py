...
from agent.svea_agent import sveaAgent,svea_cnsnAgent
...
def parse_args():
    ...
    ### train weather
    parser.add_argument('--weather', default='train',type=str, choices=['train', 'Default', 'WetCloudySunset','HardRainNoon'])
    # environment
    ...
    # train
    parser.add_argument('--agent', default='bisim', type=str, choices=['baseline', 'bisim', 'deepmdp','drqv2_cnsn','drqv2','svea','svea_cnsn','drqv2_cn'])
    ...
    # eval
    parser.add_argument('--eval_freq', default=2000, type=int,help='eval every step')  # TODO: master had 10000
...
def evaluate(env, agent, video, num_episodes, L, step, device=None, embed_viz_dir=None, do_carla_metrics=None):
    ...
    for i in range(num_episodes):
        ...
        L.log('eval/episode_reward_'+weather_name, episode_reward, step)
...
def make_agent(obs_shape, action_shape, args, device):
    ...
    elif args.agent =='svea_cnsn':
        agent = svea_cnsnAgent(
            obs_shape=obs_shape, 
            action_shape=action_shape, 
            device=device, 
            lr=1e-4,
...
def main():
    ...
    with open(os.path.join(args.work_dir, 'args_train.json'), 'w') as f:
    ...
    for step in range(args.num_train_steps):
        ...
        if step % args.eval_freq == 0 and step > 0:
            ...
            evaluate(eval_env, agent, video, args.num_eval_episodes, L, step,do_carla_metrics=True)
            if args.save_model and step==100000:
                agent.save(model_dir, step)
            if args.save_model and step==150000:
                agent.save(model_dir, step)
            if args.save_buffer:
