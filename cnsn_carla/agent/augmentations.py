...
def load_config(key=None):
    path = os.path.join(os.getcwd(),'svea_setup', 'config.cfg')
    #print(os.getcwd())
    with open(path) as f:
        data = json.load(f)
    if key is not None:
...
def random_overlay(x, dataset='places365_standard'):
	...
	if dataset == 'places365_standard':
		if places_dataloader is None:
			_load_places(batch_size=n, image_size=h)
		imgs = _get_places_batch(batch_size=n).repeat(1, (c*num_cameras)//3, 1, 1)
	else:
		raise NotImplementedError(f'overlay has not been implemented for dataset "{dataset}"')
	results = ((1-alpha)*(x/255.) + (alpha)*imgs)*255.
	results = results.reshape(n,c,num_cameras,h,-1)
	results = results.permute(0,1,3,2,4)
	results = results.reshape(n,c,h,-1)
	return results
